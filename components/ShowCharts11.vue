            ...
            <el-main>
              <el-scrollbar style="width:2.8rem" height="3rem">
                <el-table :data="ForceFirmsList" >
                  ...
                  <el-table-column prop="" align="center" label="" style="font-size:0.08rem" />
                </el-table>
              </el-scrollbar>
            </el-main>
          </el-container>
...
export default {
  ...
  methods: {
    ...
    queryForceFirms() {
      ...
      let cypher = `match p=(n:\`产业\`{name:"计算机、通信和其他电子设备制造业"})-[*1..3]->() return p`; 
    ...
    getTotalIndustryData(){
      ...
      let nodes = new Set()
      const session = this.$neo4j.getSession();
      let cypher = `match p=(n:\`产业\`{name:"计算机、通信和其他电子设备制造业"})-[*1..3]->(m:\`单位\`) return p`; 
      session.run(cypher).then(res => {
        let records = res.records;
