<template>
  ...
  <!-- 搜索框 -->
  <div style="width:22%;height:4.2rem;position:absolute;top:0.58rem;left:7.5rem">
      ...
      <html class="dark">
        <el-form :inline='true' :model="formInline" :rules="rules" ref="neo4jform" label-width="0.5rem">
          ...
          <el-form-item prop="startNode">
            <div style="position:absolute;width:2rem;left:0.1rem;top:0.7rem">
              <el-form-item 
                label="起始节点:"
                label-width=0.5rem
                size='medium'>
          ...
          <br/>
          <el-form-item prop="endNode">
            <div style="position:absolute;width:2rem;left:0.1rem;top:1.4rem">
              <el-form-item 
                label="终止节点:"
                label-width=0.5rem
                size='medium'>
          ...
          <el-form-item prop="relationName">
            <div style="position:absolute;width:2rem;left:0.1rem;top:2.1rem">
              <el-form-item 
                label="关系类型:"
                label-width=0.5rem
                size='medium'>
          ...
          <!-- 查询按钮 -->
          <el-form-item>
          <div style="position:absolute;width:0.5rem;left:0.6rem;top:2.8rem">
  ...
  data() {
    return {
      ...
      rules: {
        ...
        relationName: [{required: false, trigger: 'blur'}],  
      },
      protocol: 'bolt',
      host: 'localhost',
      port: 7687,
    ...
    query2(cypher) {
      ...
      session.run(cypher).then(res => {
        ...
        for (let i = 0; i < records.length; i ++ ) {
          ...
          let src = records[i]._fields[0].start.labels[0]
          if(src == "单位") continue
          if(src == "子类" && des == "部类") continue;
          if(src == "部类" && des == "产业") continue;
          let flag = 0 //此查询语句需要flag关系的类别
          let srctype = records[i]._fields[0].segments[flag].start.labels[0];
          if (srctype == '单位'){
            nodes.add(records[i]._fields[0].segments[flag].start.properties.name + "+"
    ...
    onSubmit() {
      this.$refs.neo4jform.validate((valid) => {
        if (valid) {
          this.connect();
          const session = this.$neo4j.getSession();
          let cypher = `match p=(n)-[r:${this.formInline.relationName}]-(m)
                        where n.name contains "${this.formInline.startNode}"
                        and m.name contains "${this.formInline.endNode}" return p`;
          let tmp = cypher.toString()
          let pos = tmp.indexOf('r')
          if(tmp[pos + 2] == ']') cypher = tmp.substr(0, pos) + tmp.substr(pos + 2)
